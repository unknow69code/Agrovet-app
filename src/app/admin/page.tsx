...
export default function AdminPage() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      enqueueSnackbar("Error al cargar la lista de administradores.", {
        variant: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `/api/auth/signupAdmin/${selectedAdmin.id}`,
        {
          method: "PUT",
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        enqueueSnackbar("Administrador eliminado correctamente", {
          variant: "success",
        });
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredAdmins.map((admin) => (
              <tr key={admin.id} className="hover:bg-gray-50">
                <td className="px-6 py-3 text-left text-xs font-medium text-gray-500">{admin.id}</td>
                <td className="px-6 py-3 text-left text-xs font-medium text-gray-500">{admin.nombre}</td>
                <td className="px-6 py-3 text-left text-xs font-medium text-gray-500">{admin.correo}</td>
