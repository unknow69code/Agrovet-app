...
// 1. Añade 'cedula_cliente' a la definición de las props
interface RegistrarPagoDeudaProps {
    id_deuda: number;
    monto_pendiente_actual: number;
    cedula_cliente: string; // <-- Prop añadida
    onPagoExitoso?: () => void;
    onClose?: () => void;
}

// 2. Recibe 'cedula_cliente' en los parámetros del componente
function RegistrarPagoDeuda({ id_deuda, monto_pendiente_actual, cedula_cliente, onPagoExitoso, onClose }: RegistrarPagoDeudaProps) {
        ...
        try {
            const response = await fetch('/api/deudas/pagar', {
                ...
                body: JSON.stringify({
                    ...
                    observaciones,
                    cedula_cliente, // <-- 3. Envía la cédula a la API
                }),
            });
